...
export default function EditarReceta() {
  ...
  async function cargarTodo() {
    ...
    const { data: subRecetasData } = await supabase.from('recetas').select('id, nombre, coste_total, produccion_gramos, tipo').eq('tipo', 'sub_receta').neq('id', recetaId).order('nombre');
    if (subRecetasData) setSubRecetas(subRecetasData);

    const { data: recetaData, error: recetaError } = await supabase.from('recetas').select('*').eq('id', recetaId).single();
    ...
    setProcedimiento(recetaData.procedimiento || '');
    
    if (recetaData.foto_url) setFotoActual(recetaData.foto_url);
    if (recetaData.produccion_gramos) setProduccionGramos(parseFloat(recetaData.produccion_gramos) || '');

    const { data: detalleData } = await supabase.from('receta_detalle').select('*').eq('receta_id', recetaId);
  ...
  async function eliminarFotoDeStorage(url: string) {
    try {
      ...
      const { error } = await supabase.storage.from('recetas-fotos').remove([fileName]);
      if (error) console.error('Error eliminando foto:', error);
    } catch (error) { console.error('Error en eliminarFotoDeStorage:', error); }
  }
  ...
  function agregarItem() {
    ...
    if (!item) return;
    setItemsEditados([...itemsEditados, { tipo: item.tipo, ingrediente_id: item.tipo === 'ingrediente' ? item.id : undefined, subreceta_id: item.tipo === 'subreceta' ? item.id : undefined, nombre: item.nombre, cantidad: cantidadActual, unidad: item.unidad, costeUnitario: item.costeUnitario, coste: cantidadActual * item.costeUnitario }]);
  ...
  function calcularCosteTotal() { return itemsEditados.reduce((sum, item) => sum + item.coste, 0); }
  ...
  async function guardarReceta() {
    ...
    if (itemsEditados.length === 0) { setMensaje('Añade al menos un ingrediente o sub-receta'); return; }
    ...
    try {
      ...
      if (fotoFile) {
        if (fotoActual) await eliminarFotoDeStorage(fotoActual);
      ...
      const { error: recetaError } = await supabase.from('recetas').update({
        nombre: nombre.trim(), tipo, procedimiento: procedimiento.trim(), porciones,
      ...
      const { error: deleteError } = await supabase.from('receta_detalle').delete().eq('receta_id', recetaId);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-900 tracking-tight">Editar Receta</h1>
                <p className="text-sm text-slate-500">Modifica los datos y composición del plato</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Foto de la receta</label>
              
              {(fotoActual && !fotoPreview) && (
                <div className="mb-3">
                  <p className="text-xs text-slate-600 mb-2 font-medium">Foto actual:</p>
                  <img src={fotoActual} alt="Foto actual" className="w-32 h-32 object-cover rounded-xl border border-slate-200 shadow-sm" />
                </div>
              )}
              
              {fotoPreview && (
                <div className="mb-3 relative group inline-block">
                  <p className="text-xs text-slate-600 mb-2 font-medium">Nueva foto:</p>
                  <img src={fotoPreview} alt="Nueva foto" className="w-32 h-32 object-cover rounded-xl border-2 border-emerald-300 shadow-sm" />
                  <button onClick={eliminarFotoActual} className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600 transition opacity-0 group-hover:opacity-100" title="Eliminar foto">
              ...
              <div className="flex items-start gap-4">
                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:bg-slate-50 hover:border-emerald-400 transition-all group">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-slate-500"><span className="font-semibold text-emerald-600">Click para cambiar</span> la foto</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          {itemsEditados.length > 0 && (
            <div className="border-t border-slate-200 pt-4">
              <h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wide">Ingredientes actuales ({itemsEditados.length})</h3>
          ...
          >
            {guardando || subiendoFoto ? (
              <>
                ...
                {subiendoFoto ? 'Subiendo imagen...' : 'Guardando cambios...'}
              </>
            ) : (
              <>
