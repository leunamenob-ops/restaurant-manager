...
export default function EditarReceta() {
  ...
  function agregarItem() {
    ...
    setItemsEditados([
      ...
      {
        tipo: item.tipo,
        ingrediente_id: item.tipo === 'ingrediente' ? item.id : undefined,
        subreceta_id: item.tipo === 'subreceta' ? item.id : undefined,
        nombre: item.nombre,
  ...
  function calcularCosteTotal() {
    return itemsEditados.reduce((sum, item) => sum + item.coste, 0);
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {tipo === 'sub_receta' ? 'Procedimiento de elaboracion' : 'Descripcion de ejecucion'}
              </label>
              <textarea
                value={procedimiento}
                ...
                rows={6}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                placeholder={tipo === 'sub_receta' 
                  ? 'Describe aqui el procedimiento de elaboracion de la sub-receta...' 
                  : 'Describe aqui el proceso de ejecucion del plato...'}
              />
              <p className="text-xs text-gray-500 mt-1">
                {tipo === 'sub_receta' 
                  ? 'Procedimiento de elaboracion de la sub-receta' 
                  : 'Descripcion del proceso de ejecucion del plato'}
              </p>
            </div>
          </div>
        </div>
