...
export default function VentasHubPage() {
  ...
  const [counts, setCounts] = useState({ platos: 0, menus: 0 });
  ...
  async function cargarDatos() {
    const [impRes, plRes, meRes] = await Promise.all([
      supabase.from('ventas_imports').select('*').order('created_at', { ascending: false }),
      supabase.from('platos').select('id', { count: 'exact', head: true }),
      supabase.from('menus').select('id', { count: 'exact', head: true }),
    ]);
    setImports(impRes.data || []);
    setCounts({ platos: plRes.count || 0, menus: meRes.count || 0 });
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-4">
        {/* ═══════════ PASO 1: PLATOS & MENÚS ═══════════ */}
        <button
          onClick={() => router.push('/ventas/menu-engineering/platos')}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-slate-900">🍽️ Platos & Menús</p>
            <p className="text-xs text-slate-500">
              Escandallos vivos desde Recetas · mapeo TPV · menús con food cost
            </p>
          </div>
          <span className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-3 py-1 shrink-0">
            {counts.platos} platos · {counts.menus} menús
          </span>
        </button>
        ...
        >
          ...
          {ultimo && (
            <span className="text-[10px] font-bold text-orange-700 bg-orange-50 border border-orange-200 rounded-full px-3 py-1 shrink-0">
              {ultimo.fecha_desde?.slice(0, 5) || ''}…{ultimo.fecha_hasta || ''}
            </span>
          )}
        </button>
