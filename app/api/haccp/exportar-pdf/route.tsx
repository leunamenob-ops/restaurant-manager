...
export async function GET(request: Request) {
  try {
    ...
    // Crear PDF con configuración simplificada
    ...
    // HEADER - Usar texto simple sin fuentes complejas
    doc.fontSize(20).text('KOST SOFTWARE', { align: 'center' });
    doc.fontSize(16).text('Reporte HACCP - Control de Puntos Críticos', { align: 'center' });
    doc.moveDown(1);
    
    doc.fontSize(12).text(`Período: ${formatearFecha(inicio)} al ${formatearFecha(fin)}`, { align: 'center' });
    ...
    doc.fontSize(11);
    doc.text(`Total Registros: ${totalRegistros}`);
    doc.text(`Registros OK: ${totalOK}`);
    doc.text(`Incidencias NO_OK: ${totalNOK}`);
    doc.text(`Cumplimiento: ${porcentajeCumplimiento}%`);
    doc.moveDown(2);
    ...
    Object.keys(registrosPorCategoria).forEach(catId => {
      ...
      doc.fontSize(13).text(catNombre, { underline: true });
      doc.moveDown(0.5);
      ...
      regs.forEach((reg: any, index: number) => {
        ...
        doc.fontSize(10);
        doc.text(`${index + 1}. ${reg.nombre_pcc} | ${fecha} | ${valor} ${unidad} | ${estado}`);
        
        if (reg.accion_correctora) {
          doc.fontSize(9).text(`   Acción: ${reg.accion_correctora}`, { indent: 20 });
        }
    ...
    if (incidencias.length > 0) {
      doc.addPage();
      doc.fontSize(14).text('INCIDENCIAS DETECTADAS', { underline: true });
      doc.moveDown(1);
      ...
      incidencias.forEach((inc: any, index: number) => {
        ...
        doc.fontSize(11);
        doc.text(`${index + 1}. ${pcc?.nombre_pcc || inc.id_pcc}`, { bold: true });
        
        doc.fontSize(10);
        doc.text(`Fecha: ${formatearFechaHora(inc.fecha_hora)}`);
        doc.text(`Valor: ${inc.valor_medido || inc.temp_final} ${inc.unidad || ''}`);
        doc.text(`Acción correctora: ${inc.accion_correctora || 'No documentada'}`);
    ...
    doc.text('Reporte generado automáticamente por KOST Software', { align: 'center' });
    doc.text('Sistema de Gestión HACCP para Hostelería', { align: 'center' });
