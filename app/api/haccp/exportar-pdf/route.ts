...
export async function GET(request: Request) {
  try {
    ...
    doc.fontSize(18).fillColor('#0e7490').text('Reporte HACCP - Control de Puntos Críticos', { align: 'center' });
    doc.moveDown(1);
    ...
    doc.fillColor(porcentajeCumplimiento >= 95 ? '#16a34a' : '#dc2626');
    ...
    Object.keys(registrosPorCategoria).forEach(catId => {
      ...
      regs.forEach((reg: any, index: number) => {
        ...
        if (reg.accion_correctora) {
          ...
          doc.text(`   Acción: ${reg.accion_correctora}`, { indent: 20 });
        }
    ...
    if (incidencias.length > 0) {
      doc.addPage();
      doc.fontSize(14).fillColor('#dc2626').text('INCIDENCIAS DETECTADAS', { underline: true });
      doc.moveDown(1);
      ...
      incidencias.forEach((inc: any, index: number) => {
        ...
        doc.text(`Valor registrado: ${inc.valor_medido || inc.temp_final} ${inc.unidad || ''}`);
