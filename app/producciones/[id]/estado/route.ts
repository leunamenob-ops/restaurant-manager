// app/api/producciones/[id]/estado/route.ts
...
// Convierte unidades de receta a unidades de compra (gr→Kg, ml→L, etc.)
function factorUnidad(u: string): number {
  ...
  return 0; // unidades no convertibles (ud, unidades, porciones)
}
...
function convertirAUnidadCompra(
  ...
  if (a === 0 || b === 0) return cantidad; // 1:1 si no se puede convertir
  return cantidad * (a / b);
}
  ...
  try {
    ...
    if (!permitidos.includes(nuevoEstado)) {
      return NextResponse.json(
        {
          ok: false,
          error: `Transición no permitida: ${prod.estado} → ${nuevoEstado}`,
        },
        { status: 400 }
      );
    }
    ...
    if (errUpdate) {
      return NextResponse.json(
        { ok: false, error: errUpdate.message },
        { status: 500 }
      );
    }
    ...
    if (debeConsumir) {
      ...
      for (const m of materiales || []) {
        ...
        // Movimiento de salida con trazabilidad del lote
